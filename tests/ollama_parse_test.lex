...
type Delta =
    TextChunk(Str)
  | ToolCallBegin(Str, Str)
  | ToolArgChunk(Str, Str)
  | FinishDelta(Str)

fn parse_tool_calls(calls :: List[jv.Json]) -> List[Delta] {
  list.fold(calls, [],
    fn (acc :: List[Delta], cj :: jv.Json) -> List[Delta] {
...
fn main() -> Str {
  ...
  # Live data test — actual gemma4:latest output with index field
