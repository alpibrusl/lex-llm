...
fn test_extract_plain_json() -> Result[Unit, Str] {
  ...
  if got == input { Ok(()) }
  else { Err(str.concat("expected passthrough, got: ", got)) }
}

fn test_extract_json_fence() -> Result[Unit, Str] {
  let got := st.extract_json_block("```json\n{\"x\":1}\n```")
  if got == "{\"x\":1}" { Ok(()) }
  else { Err(str.concat("expected unwrapped JSON, got: ", got)) }
}
...
fn test_extract_strips_outer_whitespace() -> Result[Unit, Str] {
  let got := st.extract_json_block("  ```json\n{\"x\":1}\n```  ")
  if got == "{\"x\":1}" { Ok(()) }
  else { Err(str.concat("expected trimmed JSON, got: ", got)) }
}

fn test_extract_empty() -> Result[Unit, Str] {
  let got := st.extract_json_block("")
  if got == "" { Ok(()) }
  else { Err(str.concat("expected empty, got: ", got)) }
}
...
fn run_all() -> Unit {
  let results := [
    test_collect_text_empty(),
    test_collect_text_single(),
    test_collect_text_multi(),
    test_collect_text_ignores_non_text(),
    test_extract_plain_json(),
    test_extract_json_fence(),
    test_extract_plain_fence(),
    test_extract_strips_outer_whitespace(),
    test_extract_empty(),
  ]
