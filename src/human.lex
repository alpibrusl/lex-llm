...
fn make_ask_human_tool(dash :: Str, customer :: Str) -> t.Tool {
  t.define(
    "ask_human",
    "Pause and ask the human operator a question. Use when you face a genuine choice you cannot resolve autonomously, need explicit approval, or lack information the operator can supply. Returns the operator's answer as a string. Do not use for routine decisions.",
    ask_human_params(),
    fn (args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] {
      ...
      # POST question to dashboard so it can broadcast human_question SSE
      let escaped := msg.json_escape(question)
      let post_body := str.join([
        "{\"id\":\"", qid, "\",\"customer\":\"", customer, "\",\"question\":\"", escaped, "\"}"
      ], "")
      let post_req0 := { method: "POST", url: str.concat(dash, "/ask-human"),
                         headers: map.new(), body: Some(bytes.from_str(post_body)), timeout_ms: None }
      ...
      # Long-poll: server blocks this GET for up to 60 s until operator answers
      let poll_req := http.with_timeout_ms(
        { method: "GET", url: str.join([dash, "/get-answer/", qid], ""),
          headers: map.new(), body: None, timeout_ms: None },
        65000)
      match http.send(poll_req) {
        Err(_) =>
          Err([{ path: "", code: "ask_human_timeout", message: "operator response timed out" }]),
        Ok(resp) => match bytes.to_str(resp.body) {
          Err(_)  => Ok(JStr("(no reply)")),
          Ok(answer) =>
            if str.is_empty(str.trim(answer)) {
