...
fn gemini_url(model :: Str, api_key :: Str) -> Str {
  str.concat(
    ...
    str.concat(model,
      str.concat(":streamGenerateContent?key=", api_key)))
}
...
fn build_request(
  ...
  let with_sys := match sys_opt {
    None => base,
    Some(sys) => list.concat(base, [("systemInstruction", JObj([
      ("parts", JList([JObj([("text", JStr(sys))])]))
    ]))]),
  }
  let with_tools :=
    if list.is_empty(tools) { with_sys }
    else {
      list.concat(with_sys, [("tools", JList([JObj([
        ("functionDeclarations", JList(list.map(tools, t.to_google_json)))
      ])]))])
    }
...
fn encode_messages(
  messages :: List[msg.Message]
) -> (Option[Str], List[jv.Json]) {
  let sys := list.fold(messages, None,
    fn (acc :: Option[Str], m :: msg.Message) -> Option[Str] {
      match m { msg.SystemMsg(s) => Some(s), _ => acc }
    })
  let contents := list.fold(messages, [],
    fn (acc :: List[jv.Json], m :: msg.Message) -> List[jv.Json] {
      match m {
        msg.SystemMsg(_) => acc,
...
fn encode_content(m :: msg.Message) -> jv.Json {
  match m {
    msg.UserMsg(text) =>
      JObj([("role", JStr("user")),
            ("parts", JList([JObj([("text", JStr(text))])]))]),
    msg.AssistantMsg(text, calls) =>
      if list.is_empty(calls) {
        JObj([("role", JStr("model")),
              ("parts", JList([JObj([("text", JStr(text))])]))]) 
      } else {
        JObj([("role", JStr("model")),
              ("parts", JList(list.map(calls, fn (c :: msg.ToolCall) -> jv.Json {
                JObj([("functionCall", JObj([
                  ("name", JStr(c.name)),
                  ("args", c.args),
                ]))])
              })))])
...
fn parse_parts(content :: jv.Json) -> List[d.Delta] {
  match jv.get_field(content, "parts") {
    Some(JList(parts)) =>
      list.fold(parts, [], fn (acc :: List[d.Delta], part :: jv.Json) -> List[d.Delta] {
